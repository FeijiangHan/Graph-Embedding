...
def pretrain(model, train_loader, optimizer, device, epoch, logger=None):
    ...
    for batch in train_loader:
        ...
        # print("batch: ", batch_g)
        # print("feat: ", feat)
        loss, loss_dict = model(batch_g, feat) # forward
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
...
def save_checkpoint(state, is_best, log_path):
    ...
    ckp = log_path + '/checkpoint/checkpoint.pth.tar'
    torch.save(state, ckp)
    
    if is_best:
        shutil.copyfile(ckp, log_path+'/checkpoint/model_best.pth.tar')
...
def main(cfg):
    ...
    if cfg.output_dir:
        ...
        mkdir(osp.join(cfg.output_dir, 'checkpoint'))
    ...
    graphs, (num_features, num_classes) = load_graph_classification_dataset(cfg.DATA.data_name,
                                                                            ...
                                                                            PE=False)
    # print("graph: ", graphs, (num_features, num_classes))
    ...
    for i, seed in enumerate(cfg.seeds):
        ...
        if args.resume:
            print(cfg)
            if osp.isfile(cfg.checkpoint_dir):
        ...
        for epoch in range(start_epoch, cfg.SOLVER.MAX_EPOCH):
            ...
            if ((epoch + 1) % 1 == 0) & (epoch > 1):
                ...
                save_checkpoint({'epoch': epoch + 1,
                                ...
                                is_best, log_path=cfg.output_dir)
