...
def export_embeddings(embed_list, y_list, file_path):
...
def graph_classification_evaluation(model, T, pooler, dataloader, device, logger):
    ...
    print("len_embed[0]: ", len(embed_list[0]))
    print("label_embed[0]: ", y_list[0])
    test_f1, test_std = evaluate_graph_embeddings_using_svm(T, embed_list, y_list)
    export_embeddings(embed_list, y_list, 'embeddings.json')
